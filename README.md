...
## ➡️ Task 3: 复杂密林穿越与动态避障

本任务是训练无人机在复杂未知环境中自主导航与避障能力。

### 1. 任务描述
* **初始状态**：无人机稳定出生在 1.0m 高度。环境将根据不同的课程阶段，随机生成不同距离的终点，并在场地内撒布大量随机半径的静态柱子（树林）以及高动能的动态拦截柱。
* **任务目标**：无人机依靠 24 根 2D 激光射线，在定高飞行的前提下，自主寻找缝隙绕开障碍物，以最快速度抵达终点。
* **挑战**：
  1. 极易陷入“奖励黑客 (Reward Hacking)”行为（如：原地悬停刷分、见障碍物后退倒车）。
  2. 多目标优化冲突：极速靠近终点的空气动力学需求（大倾角）与定高防侧翻的安全需求（小倾角）相互拉扯。
  3. 面对环境复杂度突变时，容易产生灾难性遗忘与策略回退。

### 2. 代码拉取与运行指南

本任务的所有核心代码均位于 `task3` 文件夹下。你可以通过以下指令获取代码：

```bash
# 如果尚未克隆仓库，请先克隆整个项目
git clone [https://github.com/0324Lw/gym-pybullet-drones.git](https://github.com/0324Lw/gym-pybullet-drones.git)
cd gym-pybullet-drones/task3
```
#### 文件结构与说明

| 文件名 | 功能说明 |
|--------|----------|
| `task3_world.py` | 物理与感知世界模型。负责无缝构建 50x50 边界、动态撒点防重叠生成柱子，以及底层 24-ray 激光雷达的射线批处理检测。 |
| `task3_env.py` | 强化学习环境核心。集成了奖励函数、严格的物理检测，并支持动态课程难度注入。 |
| `task3_train.py` | PPO 串行课程学习主脚本。采用连续调用 model.learn() 的链式训练法，实现阶段无缝跃迁与学习率动态重置。 |
| `task3_model_test.py` | 模型评估与渲染脚本。内置防内存泄露机制，自动过滤坠毁回合，支持 3D 跟随视角与 2D 雷达俯视图的低频 (10 FPS) 慢动作双视角 GIF 合成。 |
...
```
### 3. 强化学习建模 (RL Modeling)
针对复杂的防作弊与长视界稀疏奖励问题，本任务的 RL 三要素进行了极其深度的重构：

#### A. 状态空间设计 (Observation Space)
- **绝对坐标剥离与雷达融合 (34维基础特征)**：
  单帧状态包含：欧拉角 (3)、底层力矩偏置 (4)、相对目标向量 (3)、以及经过归一化的 24 维 LiDAR 探测距离。
- **核心技巧：帧堆叠 (Frame Stacking)**
  3 帧堆叠形成 102 维时序输入，赋予网络“计算障碍物相对运动速度”的动态避障能力。

#### B. 动作空间设计与平滑技巧 (Action Space & Smoothing)
- **动作映射**
  动作空间为 $[-1, 1]^4$，映射为悬停基准推力的 $\pm 50\%$ (ACTION_SCALE = 0.50)。
- **抗抽搐平滑**
  维持 EMA_ALPHA = 0.5，并在奖励中施加轻度动作突变惩罚，确保避障动作敏捷而不失工业级真实无人机的圆滑。

#### C. 奖励函数组成 (Reward Shaping - 反作弊特化版)
本任务在经历了数十次局部最优挣扎后，打磨出了无懈可击的奖励体系：
1. **步数惩罚**：微调合适值，避免无人机在终点外徘徊“刷低保”的黑客行为。
2. **高斯定高奖励**：鼓励无人机保持 1.0m 高度飞行，稍微偏离即衰减。
3. **不对称进近奖励**：前进给分，但若检测到远离终点（倒车），则施加双倍惩罚，彻底斩断其面对障碍物时“后退逃避”的退路，逼迫其横向侧飞。
4. **人工势场斥力**：靠近障碍物即会惩罚，提前感知到碰撞风险。
5. **事件奖励**：高度偏离 0.5m (DEVIATE)、倾角超过 0.4 rad (FLIP)、雷达探测极近 (LIDAR) 均结束回合并-50 分。

### 4. 算法、网络结构与超参数
#### 训练方法：分段课程学习
采用稳健的三阶段课程学习链式训练：
- **阶段 1：简单环境避障**
  环境：距离 20m，引入 5个静态障碍物。
  目的：让网络初步建立定高与靠近目标的信心，同时利用 5个柱子作为“雷达疫苗”，迫使底层网络尽早提取 LiDAR 权重，防止后续阶段面临全新视觉特征时引发灾难性遗忘。
- **阶段 2：一般环境避障**
  环境：距离 25m，10个静态障碍物 + 2个动态障碍物。
  目的：通过重新调用 model.learn()，强制将学习率瞬间拉回初始峰值 ($3 \times 10^{-4}$)，方便微调。
- **阶段 3：复杂环境避障**
  环境：距离 45m，25个静态障碍物 + 4个动态障碍物。

#### 深度网络架构与参数
- 采用深层 MLP 架构：`[512, 256, 128]` 提取 102 维高阶空间特征。
- 开启 10 进程 SubprocVecEnv，Rollout Size = 2048，Batch Size = 1024。
- target_kl=0.015 避免策略改进幅度太大。

## ➡️ Task 4: 敏捷穿梭与Sim2Real视觉控制
本任务是训练无人机基于深度视觉与本体感知，在带有物理随机化与延迟的环境中完成空间穿越任务。

### 1. 任务描述
- **初始状态**：无人机在固定起点生成。环境将随机生成一条包含5个参考点的样条曲线，并在路点处放置5个倾角随机（最高达45度）的窄门。
- **任务目标**：无人机在不依赖全局绝对坐标的前提下，仅依靠机载深度相机与本体状态，沿样条轨迹飞行，依次穿越所有倾斜窄门到达终点。
- **核心挑战**：
  - 状态空间的部分可观测性：相机视场角（FOV 110°）有限，存在视觉盲区。
  - Sim2Real 现实差距：执行器存在响应延迟、死区与物理饱和，且环境存在质量、空气动力学等参数的随机扰动与传感器噪声。
  - 奖励规避行为：智能体容易为了规避碰撞惩罚而选择原地悬停或偏离赛道缓慢漂移。

### 2. 代码拉取与运行指南
本任务的所有核心代码均位于 `task4` 文件夹下。你可以通过以下指令获取代码：

```bash
# 如果尚未克隆仓库，请先克隆整个项目
git clone [https://github.com/0324Lw/gym-pybullet-drones.git](https://github.com/0324Lw/gym-pybullet-drones.git)
cd gym-pybullet-drones/task4
```

#### 文件结构与说明
| 文件名 | 功能说明 |
|--------|----------|
| `task4_world.py` | 物理与感知世界模型。负责生成样条轨迹、倾斜门阵列、执行域随机化（质量、风阻修改），并渲染带有高斯与椒盐噪声的深度图。 |
| `task4_env.py` | 强化学习环境核心。集成了包含死区与一阶延迟滤波的执行器模型、多输入字典状态空间（Dict Space）以及基于状态机的穿门检测逻辑。 |
| `task4_train.py` | 训练主脚本。实现了非对称特征提取器（Asymmetric Features Extractor），在正向传播中对 Actor 屏蔽全局特权信息，调用 PPO 算法进行训练。 |
| `task4_model_test.py` | 模型评估与诊断脚本。用于加载训练后的模型执行确定性策略，输出每步的奖励组件数值，并支持渲染 3D 视角的 GIF 动图。 |
...
### 3. 强化学习建模 (RL Modeling)
针对视觉导航与 Sim2Real 迁移需求，本任务的强化学习建模设定如下：

#### A. 状态空间设计 (Observation Space - 非对称 Dict 空间)
采用字典空间格式，分为 Actor/Critic 共享部分与 Critic 特权部分：
- **深度视觉 (depth_img)**：`[3, 64, 64]`，3 帧堆叠的单通道深度图。
- **本体感知 (proprioception)**：`[51]`，包含 3 帧堆叠的无人机四元数 (4)、线速度 (3)、角速度 (3)、归一化电机转速 (4) 与目标门的相对局部坐标 (3)。
- **特权信息 (critic_privileged)**：`[27]`，仅供 Critic 价值评估使用，包含无人机绝对坐标、全局线速度、目标门绝对坐标与法向量，以及前方5个样条点的相对坐标。

#### B. 动作空间设计与执行器建模 (Action Space & Actuator Modeling)
- **动作映射**：动作空间为 $[-1, 1]^4$，线性映射为悬停转速的 $\pm 50\%$。
- **死区拦截**：过滤绝对值小于阈值 (0.05) 的动作输入。
- **延迟与饱和模拟**：应用一阶低通滤波（$\alpha=0.6$）模拟电机加速延迟，并限制最大转速与最小怠速。

#### C. 奖励函数组成 (Reward Shaping)
1. **步数惩罚**：每步施加固定负值，促使无人机快速前进。
2. **跟踪奖励**：计算无人机速度在样条曲线切线方向上的投影分量，并乘以基于当前位置到样条曲线距离的高斯衰减系数。
3. **姿态对齐奖励**：计算无人机局部坐标系正前方（机头）向量与目标门法向量的点积，引导机身姿态调整。
4. **动作平滑惩罚**：计算相邻两步动作差值的平方，限制推杆突变。
5. **事件奖励**：触地、侧翻超过阈值或偏离赛道将终止回合并给予固定负分；超时（达到最大步数）等同于坠毁给予负分；成功穿门给予阶梯式递增正分；完成所有门给予通关正分。

### 4. 算法、网络结构与超参数
#### 训练方法
端到端与密集奖励引导：不使用显式的外部课程学习设置，依靠环境内样条跟踪密集奖励与目标门切换状态机，直接进行端到端训练。

#### 深度网络架构与参数
采用非对称多输入 Actor-Critic 架构 (MultiInputPolicy)：
- **CNN 特征提取**：3层卷积网络对 `depth_img` 进行降维提取。
- **MLP 特征提取**：分别对 `proprioception` 和 `critic_privileged` 提取特征。
- **物理隔离机制**：在网络前向传播时，将输入 Actor 的特权特征维度置零，确保 Actor 在不获取全局信息的条件下输出策略。
- 开启 10 进程 SubprocVecEnv 并使用 VecNormalize 进行状态归一化。
...
#### 域随机化
回合重置时，无人机质量与空气动力学参数在 $\pm 10\%$ 范围内均匀采样。
